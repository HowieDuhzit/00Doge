let lastConfig = null
let sky = null
// SPEED is now configurable via app.config.cycleSpeed
let daysPassed = 0
let previousHour = 0
let cycleStartTime = Date.now()
let fogParticleSystem = null

// Sky texture cycling variables
let daySky = null
let nightSky = null
let currentSkyMode = 'day' // 'day', 'night', or 'transition'
...
// Create UI with Pip-Boy style - increased size to accommodate weather text
...
function updateDaysPassed() {
	...
	const cycleSpeed = app.config.cycleSpeed || 1.6
	const dayLengthMs = (24 / cycleSpeed) * 1000 // Convert speed to real milliseconds for a full day
...
app.on('update', delta => {
	...
	// Format time in Pip-Boy style with sky mode indicator
	let timeString = `${hours.toString().padStart(2, '0')}:${minutes.toString().padStart(2, '0')}`
	
	// Add sky transition indicator if day/night cycling is enabled
	if (app.config.useDayNightSky && app.config.autoRotate) {
		const modeIndicator = currentSkyMode === 'dawn' ? ' ☀️↗️' : 
							  currentSkyMode === 'dusk' ? ' 🌙↘️' : 
							  currentSkyMode === 'day' ? ' ☀️' : ' 🌙'
		timeString += modeIndicator
	}
...
function updateSunPosition(delta) {
	...
	if (app.config.autoRotate) {
		...
		const cycleSpeed = app.config.cycleSpeed || 1.6
		let newTime = (hour24 + cycleSpeed * delta) % 24
		previousHour = newTime

		// Update time of day with 1 decimal place
		app.config.timeOfDay = parseFloat(newTime.toFixed(1))

		// Update sky texture cycling if enabled
		if (app.config.useDayNightSky) {
			updateSkyTextureCycling(newTime, delta)
		}
...
// Sky Texture Cycling Functions
function updateSkyTextureCycling(hour24, delta) {
	if (!sky) return
	
	// Define transition periods
	const DAWN_START = 5.0    // Dawn transition begins
	const DAWN_END = 7.0      // Dawn transition ends (fully day)
	const DUSK_START = 17.0   // Dusk transition begins  
	const DUSK_END = 19.0     // Dusk transition ends (fully night)
	
	let targetTexture = null
	let shouldTransition = false
	let transitionProgress = 0
	
	// Determine what texture we should be showing
	if (hour24 >= DAWN_END && hour24 < DUSK_START) {
		// Daytime - use day texture
		targetTexture = app.config.daySkyTexture?.url
		currentSkyMode = 'day'
	} else if (hour24 < DAWN_START || hour24 >= DUSK_END) {
		// Nighttime - use night texture
		targetTexture = app.config.nightSkyTexture?.url
		currentSkyMode = 'night'
	} else {
		// Transition time
		shouldTransition = true
		
		if (hour24 >= DAWN_START && hour24 < DAWN_END) {
			// Dawn transition (night to day)
			transitionProgress = (hour24 - DAWN_START) / (DAWN_END - DAWN_START)
			currentSkyMode = 'dawn'
		...
		} else if (hour24 >= DUSK_START && hour24 < DUSK_END) {
			// Dusk transition (day to night)
			transitionProgress = (hour24 - DUSK_START) / (DUSK_END - DUSK_START)
			currentSkyMode = 'dusk'
	...
	// Update sky texture if we have a target and it's different from current
	if (targetTexture && sky.bg !== targetTexture) {
		sky.bg = targetTexture
		console.log(`Sky texture updated for ${currentSkyMode} (${hour24.toFixed(1)}h)`)
	}
	
	// Fallback to single sky texture if day/night textures aren't set
	if (!targetTexture && app.config.sky?.url && sky.bg !== app.config.sky.url) {
		sky.bg = app.config.sky.url
	}
	
	// Store transition progress for potential future use (opacity blending, etc.)
	skyTransitionProgress = transitionProgress
}

function createDualSkySystem() {
	// This function could be extended in the future to create two sky nodes
	// for true blending effects if the engine supports it
	console.log('Dual sky system initialized')
}
...
function updateWeatherUI() {
	...
	let weatherDisplay = `Weather: ${weatherName}\n${weatherDescription}`
	
	// Add zone indicator if player is in a weather zone
	if (playerWeatherOverride) {
		weatherDisplay += '\n[ZONE WEATHER]'
	}
	
	weatherText.value = weatherDisplay
}
...
// Weather zone trigger system
let weatherZones = {}
let playerWeatherOverride = null
let originalWeather = null

// Weather Zone Functions
function initializeWeatherZones() {
	// Parse weather zone configuration
	weatherZones = {}
	
	// Parse the weather zones from config
	const zonesConfig = app.config.weatherZones || ''
	if (zonesConfig.trim()) {
		try {
			const zones = zonesConfig.split('\n').filter(line => line.trim())
			zones.forEach(zone => {
				const [signal, weatherType] = zone.split(':').map(s => s.trim())
				if (signal && weatherType && WEATHER_TYPES[weatherType.toUpperCase()]) {
					weatherZones[signal] = WEATHER_TYPES[weatherType.toUpperCase()]
					console.log(`Weather zone configured: ${signal} -> ${weatherType}`)
				} else {
					console.warn(`Invalid weather zone config: ${zone}`)
				}
			})
		} catch (err) {
			console.error('Error parsing weather zones:', err)
		}
	}
	
	console.log('Weather zones initialized:', weatherZones)
}

function handleWeatherZoneTrigger(signal, data) {
	try {
		const targetWeather = weatherZones[signal]
		if (!targetWeather) return // Not a weather zone signal
		
		console.log(`Weather zone triggered: ${signal} -> ${targetWeather}`)
		
		// Check if this is a player-specific trigger
		if (data && data.playerId) {
			// Get local player
			const localPlayer = world.getPlayer()
			if (localPlayer && localPlayer.id === data.playerId) {
				console.log(`Local player entered weather zone: ${targetWeather}`)
				
				// Store original weather if not already overridden
				if (!playerWeatherOverride) {
					originalWeather = currentWeather
				}
				
				// Set weather override
				playerWeatherOverride = targetWeather
				setWeather(targetWeather, app.config)
				
				// Update UI to show zone weather
				if (weatherText) {
					weatherText.value += '\n[ZONE WEATHER]'
				}
			}
		...
		console.error('Error handling weather zone trigger:', err)
	}
}

function handleWeatherZoneExit(signal, data) {
	try {
		const targetWeather = weatherZones[signal]
		if (!targetWeather) return // Not a weather zone signal
		
		console.log(`Weather zone exit: ${signal}`)
		
		// If player was in this weather zone, restore original weather
		if (playerWeatherOverride === targetWeather) {
			console.log(`Local player exited weather zone, restoring weather: ${originalWeather}`)
			
			playerWeatherOverride = null
			
			// Restore original weather or current natural weather
			if (originalWeather) {
				setWeather(originalWeather, app.config)
				originalWeather = null
			} else if (randomWeatherEnabled) {
				// Let random weather system take over
		...
		console.error('Error handling weather zone exit:', err)
	}
}

// Listen for spherearea signals
world.on('*', (signal, data) => {
	// Handle weather zone triggers
	if (weatherZones[signal]) {
		handleWeatherZoneTrigger(signal, data)
	}
})

// Listen for area exit signals (these come as app signals with value 0)
app.on('*', (signal, value) => {
	if (weatherZones[signal] && value === 0) {
		handleWeatherZoneExit(signal, null)
	}
})
	...
	return [
		{
			...
			kind: 'texture',
			onChange: (value) => {
				if (sky && !app.config.useDayNightSky) {
					sky.bg = value?.url
				}
			}
		},
		{
			type: 'section',
			key: 'dayNightSkySection',
			label: 'DAY/NIGHT SKY TEXTURES'
		},
		{
			key: 'useDayNightSky',
			label: 'Enable Day/Night Sky Cycle',
			type: 'switch',
			options: [
				{ value: false, label: 'Off' },
				{ value: true, label: 'On' }
			],
			initial: app.config?.useDayNightSky || false,
			onChange: (value) => {
				if (value) {
					console.log('Day/Night sky cycling enabled')
					createDualSkySystem()
					// Immediately update sky based on current time
					if (app.config.autoRotate) {
						updateSkyTextureCycling(app.config.timeOfDay || 0, 0)
					}
				} else {
					console.log('Day/Night sky cycling disabled')
					// Revert to default sky texture
					if (sky && app.config.sky?.url) {
						sky.bg = app.config.sky.url
					}
				}
			}
		...
		{
			key: 'daySkyTexture',
			label: 'Day Sky Texture',
			type: 'file',
			kind: 'texture',
			onChange: (value) => {
				if (app.config.useDayNightSky && currentSkyMode === 'day') {
					if (sky) sky.bg = value?.url
				}
			}
		},
		{
			key: 'nightSkyTexture',
			label: 'Night Sky Texture',
			type: 'file',
			kind: 'texture',
			onChange: (value) => {
				if (app.config.useDayNightSky && currentSkyMode === 'night') {
					if (sky) sky.bg = value?.url
				}
			}
		},
		...
		},
		{
			...
			max: 50.0,
			step: 0.1,
			initial: app.config?.cycleSpeed || 1.6,
			dp: 3,
			onChange: (value) => {
				// Reset cycle timing when speed changes
				if (app.config.autoRotate) {
		...
		{
			key: 'weatherZones',
			label: 'Weather Zones Config',
			type: 'text',
			multiline: true,
			placeholder: 'RadZone:RADSTORM\nStormArea:STORM\nDustBowl:DUSTSTORM',
			initial: app.config?.weatherZones || '',
			onChange: () => {
				initializeWeatherZones()
			}
		},
