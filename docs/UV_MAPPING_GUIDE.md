...
- ✅ Core geometry utilities (`geometry-utils.ts`) - Created centralized functions
- ✅ All weapon meshes (`weapon-mesh-factory.ts`) - Using geometry-utils, removed duplicates
- ✅ Weapon view models - Using subdivided geometry
- ✅ Weapon pickups - Using subdivided geometry
- ✅ Level geometry (walls, floors, ceilings) - `level-builder.ts` imports from geometry-utils
- ✅ Destructibles (crates, barrels) - `level-builder.ts` buildProp() uses subdivided geometry
- ✅ Item pickups (health, armor, ammo, keys) - `pickup-system.ts` using subdivided geometry
- ✅ Props (doors, frames) - `door-system.ts` using subdivided geometry

### Status

All major textured geometry in the project now uses proper UV mapping through centralized geometry-utils functions. See [WEAPON_UV_MAPPING.md](./WEAPON_UV_MAPPING.md) for implementation details and maintenance guidelines.
...
3. **Exceptions** (when primitives are OK):
   ...
   - VFX elements (muzzle flash, explosions, particles)
...
- Level Implementation: `src/levels/level-builder.ts`
- Pickup Implementation: `src/levels/pickup-system.ts`
- Procedural Textures: `src/levels/procedural-textures.ts` and `src/weapons/weapon-textures.ts`
- Detailed Weapon Guide: `docs/WEAPON_UV_MAPPING.md`
